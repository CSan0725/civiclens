...
| **M4. 주소→지역구 정확도 end-to-end 검증** | ✅ **2026-08-27** |
| **3. 경계 전량(50개 주+DC+준주)** | ✅ **2026-08-27** — 441 지역구, 로컬+Neon+R2 |
| **3b. 관할구 유형별 프론트 (Delegate/준주 표기)** | ✅ **2026-08-28** — FC-1 위반 7건 수정 |
| **4. FEC 전량(50개 주+DC+준주)** | ✅ **2026-08-28** — 10,446명/56개 관할구, **로컬+Neon+라이브 완료** |
...
### 4단계 요약 (상세: `docs/P4-candidates-full.md`)

전량 **10,446명 / 56개 관할구**를 로컬에 적재했다(슬라이스 0의 7.4배). 전국 경로(`--states` 생략)
**290요청 · 7분 46초** — GitHub Actions 6시간 캡의 2%다.

비용 측정이 실제로 산 것은 소요시간이 아니라 **3개 주로는 볼 수 없던 결함 6건**이었다:

| | 무엇 | 처리 |
|---|---|---|
| A | openFEC이 찍는 지역구 **61–92번 20건**이 `CHECK(0..60)` 위반 → **잡이 죽는다** | `MAX_DISTRICT`로 NULL 처리 + 집계. **의석은 남긴다** |
| B | 119대에 없는 지역구 140건 (재구획 소멸 13 + 무의미 번호) | 적재하되 FR-C4 집계 |
| C·D | **같은 의석을 세 소스가 98 / 00·01 / 0으로 번호 매긴다** → DC·준주 6개 페이지가 후보 **0건** | 번호로 안 맞춘다 — 의석이 하나면 주 전체가 그 자리 |
| E | 준주 "상원" 후보 13건 (없는 자리) | 적재 + `senateSeats=0` 게이트 + 집계 |
| F | 2022 승자 4명이 openFEC 로스터에 부재 | 지어내지 않고 센다 (슬라이스 0 CA 6건과 동종) |

⚠️ **C·D는 ETL 매처에도 있었다** — 북마리아나 대표가 미연결이었다. 페이지와 매처를 같은 규칙으로 맞췄고
준주 대표 연결이 4/6 → 5/6이 됐다.

검증은 실제 페이지로 했다: DC·PR·GU·MP·VI·AS 여섯 페이지의 렌더된 후보 수가 DB와 정확히 일치
(수정 전이면 전부 0). 주 페이지 회귀 0건 — 다른 지역구 후보가 새지 않는다.

재실행으로 멱등·재개 확인: 명부 106페이지 전부 `pages_unchanged`, 수치 동일.

매칭: exact 455 / fuzzy 50. 119대 현직 하원 **393/449(87.5%)**, 상원 51/86(창 5년 < 임기 6년).
미연결 하원 54명은 전부 애칭·법적 이름 차이(수기 큐). AZ-7 Adelita Grijalva 자리에 아버지
`GRIJALVA, RAUL M`이 있는 것이 **임계값을 낮추면 안 되는 이유의 실물**이다.

**Neon 적재 완료 (2026-08-28)** — 15분 50초(로컬 7분 46초의 2배, 차이는 전부 결과 단계의 Neon 왕복).
행수 로컬과 일치(`campaign_finance`만 −2, 로컬에 남은 2c retract 잔재 빈 행 2개 — Neon이 깨끗한 쪽).
무결성·주별·cycle별 전부 동일. 매칭은 Neon 490 대 로컬 505 — **보류된 118대 구멍**이 원인이고
(2022 선거는 118대에 조인, Neon 416행 대 로컬 441행) 119대 현직 커버리지는 오히려 Neon이 높다(87.8%).

**라이브 확인 완료** — `force-dynamic`이라 재배포 없이 즉시 반영. DC 34 · PR 12 · GU 8 · MP 7 · VI 9 · AS 5,
전부 Neon 수치와 일치(**적재 직전엔 전부 0**). TX·NY·FL·OH·MT 등 새 주도 렌더.
카피는 "all 50 states…"로 바뀌고 "Other states are being added"는 사라졌다.
주소 조회도 정상(백악관 → 1198 → Norton, 상원 없음).

**P4 데이터 전량 완료.** 남은 건 Neon 118대 보강(보류), 수기 큐 55건, R2 스냅샷.

### M4 요약 (상세: `docs/P4-M4-district-accuracy.md`)

두 축으로 측정. 정답 기준은 **다른 Census 산출물**을 썼다 —
우리는 일반화된 `cb_500k`, 대조군은 지오코더가 답하는 TIGERweb 원해상도.

| 측정 | 표본 | 결과 |
|---|---:|---|
| 경계 충실도 — 내부 | 200 | **100.0%** |
| 경계 충실도 — 경계 185 m 이내(적대적 표본) | 198 | **98.0%** |
| 경계 100 m 초과 지점 | 275 | **100.0%** |
| 주소 경로 — 지역구·대표 3인 | 24 | **100.0%** |
| 주소 매칭률 | 37 | 24 = 64.9% |

**불일치 사유 분해: 경계 일반화 4건 · 지오코더 한계 13건 · 우리 결함 0건.**
불일치 4건은 전부 인접 지역구 사이, 공유 경계에서 8~54 m, 우리 판정은 자기모순 없음.
그 띠는 3개 주 면적의 **0.272%**다(면적 기준 지표이지 인구 기준 오류율이 아님).

검증이 찾아낸 **우리 쪽 결함 2건은 수정 완료**:
- `not_found` 메시지가 "전체 ZIP 코드로 시도"를 권했는데 **ZIP만으로는 절대 매칭되지 않는다**(0/3).
- 40건 중 1건이 8초 타임아웃(평시 0.3초). 타임아웃·5xx 한정 **1회 재시도** 추가 → 재실행 0건.

## 다음 단계 후보

- (A) `vote.bill_id` NULL 원인조사 — ✅ 종료.
- (B) P4 — **슬라이스 0 전 단계(1·2a·2b·2c·2d·M4) 완료.** 다음은 경계 전량(§8-E 준주 결정 선행)
  또는 FEC 전량.
