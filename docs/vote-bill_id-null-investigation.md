...
| senate | 831 | **269** | 562 | **62.6%** (269/430, 지명·조약 401건 분모 제외) |
...
### 상원 잔여 미링크 562건의 분해 — 구조적 401 + **수정 가능한 갭 161**

`verify_links.mjs`의 question 기준 분류는 상원에서 무의미하다(상원은 실질 정보가 `vote_type`에 있다).
`vote_type` 기준으로 다시 분해했다:

| 구분 | 표결 | 링크됨 | 미링크 | 성격 |
|---|---:|---:|---:|---|
| 지명·조약 (PN/TREATYDOC) | 401 | 0 | 401 | **구조적** — 법안이 존재하지 않음 |
| 법안을 지목 | 375 | 264 | 111 | 아래 참조 |
| 법안을 지목하지 않음 | 55 | 5 | 50 | 아래 참조 |

**미링크 161건은 전부 수정안(amendment) 표결이다.** 분포가 정확히 갈린다:

| | `amendment_number` | 건수 |
|---|---|---:|
| 미링크(지명·조약 제외) | **전부 NOT NULL** | 161 |
| 링크됨 | **전부 NULL** | 269 |

즉 **"수정안 표결이면 반드시 미링크, 아니면 반드시 링크"** 로 100% 갈린다. 우연이 아니다.

**카탈로그 갭은 0건이다.** 미링크 중 모(母)법안을 지목한 111건은 **전부 그 법안을 우리가 보유**하고 있다
(예: `S.Amdt. 473 to S.Con.Res. 7` — sconres 7은 카탈로그에 있음). 나머지 50건은 예산결의 관련
point of order·Section 302/313 waiver 동의로, `vote_type`에 모법안 번호가 아예 없다.

**원인(추정, 미확정).** `_resolve_bill`(`senate_xml_sync.py:106`)은 XML의 `<document>` 엘리먼트만 읽는다.
수정안 표결에서는 `<document>`가 비어 있고 모법안이 `<amendment>` 하위의
**`<amendment_to_document_number>`** 에 들어가는 것으로 보인다 — 이 필드는 픽스처
(`senate_vote_119_2_00231.xml`)의 스키마에 실재하지만 파서가 전혀 읽지 않는다.
**직접 확인은 못 했다**: 실제 수정안 표결 XML을 받아봐야 하는데 senate.gov가 이 네트워크에서 403이다.
확인하려면 Actions에서 수정안 표결 1건의 원본 XML을 떠서 `<document>`/`<amendment_to_document_number>`를 봐야 한다.

**이건 구조적 한계가 아니라 고칠 수 있는 갭이다.** 고치면 상원 링크율은 62.6% → 최대 100%(430/430)가 되고,
법안 상세 페이지가 "이 법안에 대한 수정안 표결"을 보여줄 수 있게 된다. 스키마는 이미 `bill_id`와
`amendment_number`를 둘 다 갖고 있어 의도 자체는 원래 그쪽이었던 것으로 보인다.

> 참고: 링크 로직이 정상임은 이미 독립적으로 확인됐다. daily 크론이 05:29Z에 수집한 상원 표결 50건 중
> 10건이 즉시 bill_id를 해석했고, 그 시점 카탈로그는 55%만 차 있었다.
...
- **상원 수정안 표결 161건 링크** — `_resolve_bill`이 `<amendment_to_document_number>`를 읽도록 확장.
  선행 작업: Actions에서 수정안 표결 XML 1건을 떠서 실제 필드 구조 확인(로컬은 403이라 불가).
